...
import { DashSucComponent } from '../../It25pts/DashSuc/DashSuc.component';
...
export default class Ev25ptsSucComponent implements OnInit {
  public window: any;
...
consultar25ptsSucursales()
{
  ...
  this.loading= true;
  this.apiserv.get25ptsSucursales(JSON.stringify(sucursales),this.formatDate(this.fechafin)).subscribe({
   next: data => {

    this.data25ptsSucursales = data; 
      console.log(data)
...
consultarinicioaycSucursales()
{
  ...
  this.loading= true;
  this.apiserv.getIniciosAYCSucursales(JSON.stringify(sucursales),this.formatDate(this.fechafin)).subscribe({
   next: data => {
